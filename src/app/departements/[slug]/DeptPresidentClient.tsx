          ...
          <div className="md:col-span-1">
            <div className="rounded-[2.5rem] overflow-hidden border border-border dark:border-slate-800 bg-card dark:bg-slate-900 shadow-xl">
              ...
              <div className="p-6">
                ...
                <p className="mt-2 flex items-center gap-1.5 text-sm font-bold text-muted-foreground dark:text-slate-300"><MapPin size={14} className="text-rose-500" />{p.dep_name}</p>

                <div className="mt-4 space-y-3 text-sm">
                  {p.mandate_since && <div><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Président depuis</p><p className="font-bold text-foreground dark:text-white">{frDate(p.mandate_since)}</p></div>}
                  {(bio.profession || p.csp) && <div><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Profession</p><p className="font-bold text-foreground dark:text-white capitalize">{bio.profession || p.csp}</p></div>}
