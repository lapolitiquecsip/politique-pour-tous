...
function MayorContent() {
  ...
  useEffect(() => {
    let active = true;
    (async () => {
      const res = insee ? await api.getMayorByInsee(insee) : slug ? await api.getMayorBySlug(slug) : null;
      if (active) setP(res ?? null);
    })();
    return () => { active = false; };
  }, [insee, slug]);

  if (p === undefined) return (
    <div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-rose-500" /></div>
  );
  if (p === null) return (
          ...
          <div className="md:col-span-2 space-y-8">
            ...
            <section className="rounded-[2.5rem] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8">
              ...
              {hasStructured ? (
                ...
                <p className="text-sm italic text-slate-400">Aucune biographie publique disponible pour ce maire à ce jour. Les informations vérifiées (fonction, mandat, étiquette, situation judiciaire) restent affichées ci-dessus.</p>
              )}
            </section>
          </div>
        </div>
