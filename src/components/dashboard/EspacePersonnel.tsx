...
/**
 * Mode d'affichage de l'espace personnel.
 *
 * « espace » : le travail — alertes, fils Pro, votes, élus suivis, territoires.
 * « compte » : les réglages — adresse, mot de passe, déconnexion.
 *
 * Un abonné Pro voit les deux séparément : son espace de travail prend la place de
 * la page d'offre, dont il n'a plus l'usage, et le tableau de bord se réduit aux
 * réglages. Pour tous les autres, les deux restent réunis sur le tableau de bord.
 */
...
export default function EspacePersonnel({ mode = "tout" }: { mode?: ModeEspace }) {
  ...
  useEffect(() => {
    // En mode « compte », rien de tout cela n'est affiché : on ne va pas chercher
    // les votes, les élus suivis et les favoris pour les laisser dans un coin.
    if (!userId || authLoading || mode === "compte") return;
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
             ...
             <div className="flex flex-col items-center sm:items-start">
                ...
                <p className="text-slate-400 text-sm mt-3 font-medium">
                  {mode === "compte"
                    ? "Vos identifiants et les réglages de votre abonnement."
                    : mode === "espace"
