...
export default function IssuesVotesView() {
  ...
  return (
    <div className="mt-8">
      <div className="mb-2 text-4xl font-staatliches uppercase text-foreground md:text-6xl">Les votes par enjeu</div>
      <p className="mb-6 text-muted-foreground">Choisissez un thème : voyez les <b>votes clés</b> de l&apos;Assemblée et comment chaque groupe s&apos;est prononcé.</p>
      ...
      {/* Légende */}
      <div className="mt-6 flex flex-wrap items-center gap-4 text-[11px] font-black uppercase tracking-widest text-muted-foreground">
      ...
      {currentIssue?.proposition && (
        <p className="mt-3 rounded-2xl border-l-4 border-slate-300 bg-muted px-4 py-3 text-sm italic text-muted-foreground">
        ...
        {filtered.map(v => {
          ...
          return (
            <div key={v.id} className="rounded-[1.5rem] border border-border bg-card p-5">
              ...
              <h3 className="mt-2 text-lg font-bold leading-snug text-foreground">{cleanTitle(v.title, v.objet)}</h3>
